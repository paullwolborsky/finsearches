...
$listData = $db->sqa("SELECT * FROM {mytinytodo_lists} WHERE field_id = ?", array($field_id));
...
$data = array();
$q = $db->dq("SELECT * FROM {mytinytodo_todos} WHERE list_id = ? $sqlWhere ORDER BY ". $listData['_uid_field'] ." DESC LIMIT 100", array($listId));
