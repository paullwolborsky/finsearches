...
{
	$s = /*chr(0xEF).chr(0xBB).chr(0xBF).*/ "Completed,Priority,Task,Contact,Date,Type,Notes,Tags,Due,DateCreated,DateCompleted\n";
	foreach($data as $r)
	{
...
function printICal($listData, $data)
{
	...
	foreach($data as $r)
	{
		...
		if ($r['tags'] != '') $descr[] = Lang::instance()->get('tags'). ": ". str_replace(',', ', ', $r['tags']);
		if ($r['note'] != '') $descr[] = Lang::instance()->get('note'). ": ". $r['note'];
		if ($r['c_contact'] != '') $descr[] = "contact: " . $r['c_contact'];
		if ($r['c_date'] != '') $descr[] = "contact Date: " . $r['c_date'];
		if ($r['c_type'] != '') $descr[] = "contact Type: " . $r['c_type'];
		if ($descr) $a[] = utf8chunks("DESCRIPTION:". str_replace("\n", '\\n', implode("\n",$descr)));
		$a[] = "END:VEVENT\r\n";
		$s .= implode("\r\n", $a);
	}
