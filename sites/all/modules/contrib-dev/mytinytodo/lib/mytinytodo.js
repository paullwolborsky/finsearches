...
var objPrio = {};
var selTask = 0;
var flag = { tagsChanged:true, readOnly:false, editFormChanged:false };
...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		if (t.options.showdate) $('#page_tasks').addClass('show-inline-date');
		if (t.options.singletab) $('#lists .mtt-tabs').addClass('mtt-tabs-only-one');
		...
		$('#task').keydown(function(event){
			...
			$('#mtt_toolbar').addClass('mtt-intask');
		}).focusout(function(){
			if('' == $(this).val()) $('#task_placeholder').addClass('placeholding');
			$('#mtt_toolbar').removeClass('mtt-intask');
		});
		...
		.keydown(function(event){
			...
			$('#mtt_toolbar').addClass('mtt-insearch');
			$(this).focus();
		}).focusout(function(){
			$('#mtt_toolbar').removeClass('mtt-insearch');
		});
		...
		//Priority popup
		$('#priopopup span').click(function(){
			prioClick($(this).attr('data-prio'),this);
		});
		...
		$("#duedate, #contactdate").datepicker({
			...
			showOn: 'button',
			buttonImage: base_url + _mtt.templateUrl + 'images/calendar.png', buttonImageOnly: true,
			constrainInput: false,
			duration:'',
			dayNamesMin:_mtt.lang.daysMin, dayNames:_mtt.lang.daysLong, monthNamesShort:_mtt.lang.monthsLong
		});

		$("#edittags").autocomplete(base_path + 'mytinytodo/ajax' + add_args + 'suggestTags', {scroll: false, multiple: true, selectFirst:false, max:8, extraParams:{fid:field_id, list:function(){ var taskId = document.getElementById('taskedit_form').id.value; return taskList[taskId].listId; }}});
	...
	loadLists: function(onInit)
	{
		...
		this.db.loadLists(null, function(res)
		{
			...
			if(res && res.total)
			{
				...
				// set alwaysShowCompl setting
				for(var i=0; i<res.list.length; i++) {
                                        res.list[i].alwaysShowCompl = res.list[i].showCompl;
                                }
...
function prepareTaskStr(item, noteExp)
{
	...
	return '<li id="taskrow_'+id+'" class="' + (item.compl?'task-completed ':'') + item.dueClass + (item.note!=''?' task-has-note':'') +
		...
		'<div class="task-middle"><div class="handle"></div><div class="task-through-right">'+prepareDuedate(item)+
...
function listMenuClick(el, menu)
{
	...
	switch(el.id) {
		...
		case 'btnAlwaysShowCompleted': alwaysShowCompletedToggle(); break;
...
function editTask(id)
{
	...
	contacttypes = JSON.parse(item.contacttype);
	...
	form.contactdate.value = item.contactdate;
	form.contacttypephone.checked = contacttypes.phone;
	form.contacttypeemail.checked = contacttypes.email;
	form.contacttypevoicemail.checked = contacttypes.voicemail;
	form.contacttypeother.checked = contacttypes.other;
	form.contacttypeotherreason.value = contacttypes.reason;
...
function saveTask(form)
{
	...
	contactMethods = {
		phone: 			form.contacttypephone.checked ? 1 : 0,
		voicemail:	form.contacttypevoicemail.checked ? 1 : 0,
		email:			form.contacttypeemail.checked ? 1 : 0,
		other:			form.contacttypeother.checked ? 1 : 0,
		reason:			form.contacttypeotherreason.value
	};
...
function tabmenuOnListSelected(list)
{
	if(list.published) {
        ...
        if (list.alwaysShowCompl) {
            $('#btnAlwaysShowCompleted').addClass('mtt-item-checked');
            $('#btnShowCompleted').addClass('mtt-item-checked');
            $('#btnShowCompleted').addClass('mtt-item-disabled');
        } else {
            $('#btnAlwaysShowCompleted').removeClass('mtt-item-checked');
            $('#btnShowCompleted').removeClass('mtt-item-disabled');
...
function alwaysShowCompletedToggle()
{
	var act = curList.alwaysShowCompl ? 0 : 1;
	curList.alwaysShowCompl = tabLists.get(curList.id).alwaysShowCompl = act;

        if (act) {
            curList.showCompl = tabLists.get(curList.id).showCompl = 1;
            $('#btnAlwaysShowCompleted').addClass('mtt-item-checked');
            $('#btnShowCompleted').addClass('mtt-item-checked');
            $('#btnShowCompleted').addClass('mtt-item-disabled');
            loadTasks();
        ...
        _mtt.db.request('setShowCompletedTasks', {list:curList.id, compl:act});
};
...
function exportCurList(format)
{
	...
	parent.location.href = base_path + 'mytinytodo/export' + add_args + 'list=' + curList.id + '&format=' + format + '&fid=' + field_id;
};

function feedCurList()
{
	if(!curList) return;
	parent.location.href = base_path + 'mytinytodo/feed' + add_args + 'list=' + curList.id + '&fid=' + field_id;
}
...
function showSettings()
{
	...
	$('#page_ajax').load(base_path + 'mytinytodo/settings' + add_args + 'ajax=yes&fid=' + field_id,null,function(){ 
...
{
	if(!frm) return false;
	var params = { save:'ajax', fid: field_id };
	$(frm).find("input:text,input:checked,select").filter(":enabled").each(function() { params[this.name || '__'] = this.value; }); 
	$(frm).find(":submit").attr('disabled','disabled').blur();
	$.post(base_path + 'mytinytodo/settings', params, function(json){
		if(json.saved) {
